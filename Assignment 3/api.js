...
exports.CreateTaskController = [
  ...
  async (req, res) => {
    ...
    // Handle `appAcronym` as fallback for `task_app_acronym`
    const taskAppAcronym = req.body.task_app_acronym || req.body.appAcronym
    req.body.task_app_acronym = taskAppAcronym // Ensure `task_app_acronym` is set in `req.body`

    if (!taskAppAcronym) {
      return res.status(400).json({
        MsgCode: "P_001",
        remarks: "Missing mandatory keys: task_app_acronym"
      })
    }
    ...
    try {
      ...
      if (!login || !bcrypt.compareSync(password, login.password) || login.accountStatus.toLowerCase() !== "active") {
        return res.status(401).json({
          MsgCode: "I_001",
          remarks: "Invalid credentials. User not authenticated."
        })
      }

      // **T_002**: Validate application existence
      if (!task_app_acronym || typeof task_app_acronym !== "string" || task_app_acronym.length > maxLength.task_app_acronym) {
        return res.status(400).json({
          MsgCode: "T_002",
      ...
      const [[app]] = await db.execute("SELECT App_permit_Create, App_Rnumber FROM application WHERE App_Acronym = ?", [task_app_acronym])
      ...
      if (task_plan) {
        const [planArray] = await db.execute("SELECT DISTINCT Plan_MVP_name FROM plan WHERE Plan_app_Acronym = ?", [task_app_acronym])
      ...
      const [[{ App_Rnumber }]] = await db.execute("SELECT App_Rnumber FROM application WHERE App_Acronym = ? FOR UPDATE", [task_app_acronym])

      const newTaskId = `${task_app_acronym}_${App_Rnumber + 1}`
      ...
      try {
        await connection.execute("INSERT INTO task (Task_id, Task_name, Task_description, Task_notes, Task_plan, Task_app_Acronym, Task_state, Task_creator, Task_owner, Task_createDate) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", [newTaskId, task_name, task_description || "", initialNotes, task_plan || "", task_app_acronym, "Open", username, username, createdate])

        await connection.execute("UPDATE application SET App_Rnumber = ? WHERE App_Acronym = ?", [App_Rnumber + 1, task_app_acronym])
...
exports.GetTaskByStateController = [
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (!user || !(await bcrypt.compare(password, user.password)) || user.accountStatus !== "active") {
        return res.status(401).json({ code: "I_001", remarks: "Invalid credentials. User not authenticated." })
      }
...
exports.PromoteTask2DoneController = [
  ...
  async (req, res) => {
    ...
    try {
      ...
      // **T_003**: Validate task existence and state
      const [[task]] = await db.execute("SELECT Task_state, Task_app_Acronym, Task_notes FROM task WHERE Task_id = ? AND Task_state = 'Doing'", [Task_id])
      if (!task) {
        return res.status(400).json({ code: "T_003", remarks: "Invalid state transition. Task must be in 'Doing' state." })
      }

      // **T_002**: Validate application and permissions for the `Done` state
      ...
      if (!app || !app.App_permit_Done) {
        return res.status(403).json({ code: "I_002", remarks: "No permissions defined for the 'Done' state." })
      }
